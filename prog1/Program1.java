...
*
*  Creates Lattice objects for each lattice in the lattice list input
file
...
*  lmScale                a non-negative number that specifies how much
to weight
*  the "language model" score, relative to the
*  "acoustic model" score
*  outputDir              a directory where output lattices and dot files
will
...
// Read through latticeListFilename
        ...
        try {
            input = new java.util.Scanner(new
                    java.io.File(latticeListFilename));
...
// Decode, print best hypothesis and various statistics
            ...
            System.out.println("WER : " + new
                    java.text.DecimalFormat("0.000").format(WER));
            totalWER += WER;
            System.out.println("Number of unique paths: " +
                    lattice.countAllPaths());
            System.out.println("Lattice density: " + new
                    java.text.DecimalFormat("0.000").format(lattice.getLatticeDensity()));
            java.util.HashSet<String> words =
                    lattice.uniqueWordsAtTime(0.5);
            printWordSet(words,outputDir + slash +
                    lattice.getUtteranceID() + ".wordsAtTime");
...
// Write lattice to output dir in dot and lattice formats
            lattice.writeAsDot(outputDir + slash +
                    lattice.getUtteranceID() + ".dot");
            String latticeOutputFilename = outputDir + slash +
                    lattice.getUtteranceID() + ".lattice";
    ...
    private static void printReference(String refFilename) {
        ...
        try {
            refInput = new java.util.Scanner(new
                    java.io.File(refFilename));
        } catch( java.io.FileNotFoundException e ) {
            System.err.println("Error: Unable to open file " +
                    refFilename);
            System.exit(1);
    ...
    private static void printWordSet(java.util.HashSet<String> words,
                                     String outFilename) {
        if( words == null ) {
            return;
        }
